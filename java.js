...
function type() {
    ...
    }, sentences[0].length * 100 + 500); // Wait for 0.5 seconds after typing the first sentence
    setTimeout(function() {
      typeSentence(sentences[2], bottomTextContainer);
    }, (sentences[0].length + sentences[1].length) * 100 + 500); // Wait for 0.5 seconds after typing the second sentence
    setTimeout(function() {
      document.addEventListener("keydown", handleKeyDown); // Listen for key press after "Press any key to continue" is typed
    }, (sentences[0].length + sentences[1].length + sentences[2].length) * 100 + 1000); // Wait for 1 seconds after typing the "Press any key to continue" sentence
}
